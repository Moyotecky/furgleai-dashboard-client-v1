...
export default function SignUpForm() {
  ...
  const handleUsernameChange = (val: string) => {
    setUsername(val);
    const result = signUpSchema.shape.username.safeParse(val);
    setErrors((prev) => ({
      ...prev,
  ...
  const handlePasswordChange = (val: string) => {
    setPassword(val);
    const result = signUpSchema.shape.password.safeParse(val);
    setErrors((prev) => ({
      ...prev,
  ...
  return (
    <div className="w-full max-w-[420px] flex flex-col justify-center px-4 py-8 md:py-12 bg-white text-zinc-950 font-sans min-h-screen">
      ...
      <div className="flex flex-col gap-1.5 mb-8 text-left">
        ...
        >
          Sign up to FurgleAI Security OS today. Secure your pipelines and automate mitigations in seconds.
        </motion.p>
      </div>
      ...
      >
        <SocialButton provider="google" onClick={() => console.log('Google Auth')} />
        <SocialButton provider="apple" onClick={() => console.log('Apple Auth')} />
      </motion.div>
      ...
      <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-5 mt-4 text-left">
        ...
        <motion.div
          initial={{ opacity: 0, y: 15 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6, delay: 0.4, cubicBezier: [0.16, 1, 0.3, 1] }}
          className="mt-2"
        >
          <button
            type="submit"
